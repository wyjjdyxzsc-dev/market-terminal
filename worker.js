...
} = globalThis.MarketTerminalAiProviderRegistry;
const {
  runVerifiedPipeline,
  getRuntimeMetadata,
} = globalThis.MarketTerminalAiVerification;
const {
...
} = globalThis.MarketTerminalAiTaskPolicy;
...
async function fetchCompanyHeadlines(env, query, limit = 14) {
...
//  Low-risk synthesis retains bounded batch racing. Policy-governed analysis
//  uses sequential generation plus a different-provider/different-model
//  verifier, with explicit latency, token, call-count, and cost-unit budgets.
// ════════════════════════════════════════════════════════════════════════

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
const _providerHealth = createProviderHealthState();

function boundedOutputTokens(provider, requested) {
  return Math.max(1, Math.min(
    Number(requested) || 2_000,
    Number(provider.maxOutputTokens) || 2_000
  ));
}

async function openaiCompatCall(provider, env, system, user, signal, maxOutputTokens) {
  const model = resolveProviderModel(provider, env);
  const body = {
    model,
    messages: [{ role: 'system', content: system }, { role: 'user', content: user }],
    temperature: 0.4,
  };
  body[provider.outputTokenParam || 'max_tokens'] = boundedOutputTokens(provider, maxOutputTokens);
  if (provider.supportsJsonMode !== false) body.response_format = { type: 'json_object' };
  ...
  if (!res.ok) {
    ...
    const err = new Error(`${provider.name} ${res.status}: ${body.slice(0, 160)}`);
...
async function geminiCall(provider, env, system, user, signal, maxOutputTokens) {
  const model = resolveProviderModel(provider, env);
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${env.GEMINI_API_KEY}`;
  const generationConfig = {
    maxOutputTokens: boundedOutputTokens(provider, maxOutputTokens),
    responseMimeType: 'application/json',
  };
  if (provider.supportsSamplingParameters !== false) generationConfig.temperature = 0.4;
...
async function cohereCall(provider, env, system, user, signal, maxOutputTokens) {
  const model = resolveProviderModel(provider, env);
  const res = await fetch(provider.endpoint, {
    method: 'POST',
    signal,
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: 'system', content: system }, { role: 'user', content: user }],
      temperature: 0.4,
      max_tokens: boundedOutputTokens(provider, maxOutputTokens),
    }),
  });
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    const error = new Error(`cohere ${res.status}: ${body.slice(0, 160)}`);
    error.status = res.status;
    throw error;
  }
  const data = await res.json();
  const text = data.message?.content?.map((part) => part?.text || '').join('') || data.text || '';
...
async function cfAICall(provider, env, system, user, _signal, maxOutputTokens) {
  const model = resolveProviderModel(provider, env);
  ...
  return {
    text,
    usage: typeof out === 'object' ? out.usage || null : null,
    requestedModel: model,
    servedModel: typeof out === 'object' && out.model ? out.model : model,
  };
}

async function callProvider(provider, env, system, user, signal, maxOutputTokens = 2_000) {
  recordProviderAttempt(_providerHealth, provider.name);
  try {
    let result;
    if (provider.format === 'gemini') {
      result = await geminiCall(provider, env, system, user, signal, maxOutputTokens);
    } else if (provider.format === 'cohere') {
      result = await cohereCall(provider, env, system, user, signal, maxOutputTokens);
    } else if (provider.format === 'cfai') {
      result = await cfAICall(provider, env, system, user, signal, maxOutputTokens);
    } else {
      result = await openaiCompatCall(provider, env, system, user, signal, maxOutputTokens);
    }
    recordProviderSuccess(_providerHealth, provider.name);
    return result;
  } catch (error) {
    if (error?.name !== 'AbortError') {
      recordProviderFailure(_providerHealth, provider.name, error?.message || error);
    }
    throw error;
  }
}
...
function providerPool(env) {
  return listProviders({ runtime: 'worker' })
    .filter((provider) => isProviderConfigured(provider, env))
    .map((provider) => ({
      ...provider,
      requestedModel: resolveProviderModel(provider, env),
      call: (e, s, u, signal, maxOutputTokens) =>
        callProvider(provider, e, s, u, signal, maxOutputTokens),
    }));
}
...
async function parkProvider(env, name, ms) {
  ...
  const cooldownUntil = Math.max(Number(cd[name]) || 0, Date.now() + ms);
  cd[name] = cooldownUntil;
...
function raceBatch(env, batch, system, user, validate) {
  return new Promise((resolve) => {
    ...
    batch.forEach((pv, i) => {
      Promise.resolve()
        .then(() => pv.call(env, system, user, controllers[i].signal, 4_000))
        .then((response) => {
...
async function runAIJson(env, system, user, validate = () => true, options = {}) {
  const preparation = options && options.policy ? options : null;
  const taskPolicy = preparation ? preparation.policy : options;
  const permittedNames = Array.isArray(taskPolicy.permittedProviderNames) && taskPolicy.permittedProviderNames.length
    ? new Set(taskPolicy.permittedProviderNames)
    : null;
  const pool = providerPool(env).filter((provider) =>
    (!taskPolicy.providerTier || provider.tier === taskPolicy.providerTier) &&
    (!taskPolicy.riskClass || isEligibleForTask(provider, {
      runtime: 'worker',
      tier: taskPolicy.providerTier,
      riskClass: taskPolicy.riskClass,
    })) &&
    (!permittedNames || permittedNames.has(provider.name))
  );
  ...
  if (!active.length && !preparation) active = pool;
  if (!active.length) throw new Error('All policy-approved AI providers are cooling down.');
  ...
  if (preparation) {
    try {
      return await runVerifiedPipeline({
        preparation,
        // Verified tasks favor the registry's deterministic reliability order.
        // Round-robin rotation can make a weaker generator consume the budget
        // before a known structured-output provider and its verifier run.
        providers: active,
        system,
        user,
        validate,
        extractJson,
        callProvider: (provider, callSystem, callUser, signal, maxOutputTokens) =>
          provider.call(env, callSystem, callUser, signal, maxOutputTokens),
        onProviderFailure: async (provider, error) => {
          if (isRateLimited(error)) await parkProvider(env, provider.name, cooldownMs(error));
        },
      });
    } catch (error) {
...
// per-item `degraded` flag survives JSON serialization through the KV cache.
...
async function fetchSituation(env) {
  ...
  try {
    const data = await runAIJson(
      env,
      withGrounding(SITUATION_SYSTEM, preparation),
      userPrompt,
      (d) => d && Array.isArray(d.domains) && d.domains.length >= 3 && d.defcon != null && validateGroundedOutput(preparation, d),
      preparation
    );
...
async function fetchAnalysis(env) {
  ...
  try {
    const data = await runAIJson(env, withGrounding(ANALYSIS_SYSTEM, preparation), userPrompt, validate, preparation);
    return attachPolicy(preparation, data, {
...
async function fetchCompany(env, query) {
  ...
  if (!ticker) {
    try {
      const search = await finnhub(env, '/search', { q: query });
  ...
  const headlines = await fetchCompanyHeadlines(env, ticker || query);
  ...
  try {
    const data = await runAIJson(env, withGrounding(COMPANY_SYSTEM, preparation), userPrompt, validate, preparation);
...
// MT2-5 Task 5: tier-2 (AI-extracted, live) relationships. See server.js's
// identical function for full rationale; this mirrors it using the Worker's
// runAIJson(env, ...) in place of Express's raceProviders(...).
...
async function fetchNexusTier2Relationships(env, companyId) {
  ...
  const evidenceItems = [
    ...
    ...await fetchCompanyHeadlines(env, instrument.symbol).catch(() => []),
  ];
  ...
  let data;
  try {
    data = await runAIJson(env, withGrounding(NEXUS_TIER2_SYSTEM, preparation), userPrompt, validate, preparation);
  } catch (error) {
    return fallback(describeAiFailure(error, preparation.policy));
  }
...
// Kept byte-identical to server.js's copy.
...
// tier-1/tier-3 generator already guards on this set; the live tier-2 path needs the
// same guard so a model cannot mint an edge to a plausible-looking ticker that does
...
// Free-text (ticker or company name) → canonical registry id, using the same ranked
// search every other consumer uses. Shared by the deprecated /api/intel/supplychain
// alias so it genuinely resolves to NEXUS data.
...
// The canonical company payload: snapshot node + tier-1/tier-3 edges + the cached live
// tier-2 layer. Shared by /api/nexus/company and the deprecated /api/intel/supplychain
// alias, so the alias returns real NEXUS data rather than the old abstaining stub.
async function nexusCompanyPayload(env, ctx, id) {
  const result = nexusCompanyDetail(id);
  if (!result) return null;
  const { data: tier2, fresh } = await getData(env, ctx,
    `nexus:tier2:${AI_TASK_POLICY_SCHEMA_VERSION}:${nexusCore.NEXUS_SCHEMA_VERSION}:${id}`,
    () => fetchNexusTier2Relationships(env, id), 900 * 1000);
...
async function fetchDeepDive(env, query, marketId = 'US') {
  ...
  let data;
  try {
    data = await runAIJson(
      env,
      withGrounding(DEEPDIVE_SYSTEM, preparation),
      userPrompt,
      validate,
      preparation
    );
...
const MAX_ALERTS = 40;
const ALERT_STATE_KEY = `alerts:state:${AI_TASK_POLICY_SCHEMA_VERSION}`;
...
const toAlert = (item) => ({
  ...
  category: item.category || 'macro', source: item.source || '', sourceUrl: item.sourceUrl || '', marketImpact: item.marketImpact || '',
...
async function detectAlerts(env, items) {
  ...
  try { const raw = await env.MT_KV.get(ALERT_STATE_KEY); if (raw) state = JSON.parse(raw); } catch {}
  ...
  state.primed = true;
  await env.MT_KV.put(ALERT_STATE_KEY, JSON.stringify(state)).catch(() => {});
...
async function handleApi(request, env, ctx, url) {
  ...
  if (p === '/api/intel/analysis') {
    try { const { data, fresh } = await getData(env, ctx, `analysis:${AI_TASK_POLICY_SCHEMA_VERSION}`, () => fetchAnalysis(env)); return json({ cached: !fresh, ...data }); }
    catch (err) { return json({ error: true, message: friendlyError(err) }, 500); }
  }
  if (p === '/api/intel/company') {
    ...
    try { const { data, fresh } = await getData(env, ctx, `company:${AI_TASK_POLICY_SCHEMA_VERSION}:${query.toLowerCase()}`, () => fetchCompany(env, query)); return json({ cached: !fresh, ...data }); }
    catch (err) { return json({ error: true, message: friendlyError(err) }, 500); }
  }
  // Deprecated alias of /api/nexus/company (shared/api-contract.js DEPRECATED_ALIASES).
  // Keeps the legacy ?q= free-text contract, resolves it through the same ranked
  // registry search, and returns the same payload shape as /api/nexus/company.
  if (p === '/api/intel/supplychain') {
    const query = (qs.get('q') || '').trim().slice(0, 60);
    if (!query) return json({ error: true, message: 'Missing company name or ticker.' }, 400);
    const id = nexusResolveQuery(query, qs.get('market') || '');
    if (!id) return json({ error: true, message: 'No NEXUS registry company matches that name or ticker.', query }, 404);
    try {
      const payload = await nexusCompanyPayload(env, ctx, id);
      if (!payload) return json({ error: true, message: 'unknown company id', id }, 404);
      return json({ deprecated: true, successor: '/api/nexus/company', query, ...payload },
        200, { Deprecation: 'true', Link: '</api/nexus/company>; rel="successor-version"' });
    } catch (err) { return json({ error: true, message: friendlyError(err) }, 500); }
  }
  if (p === '/api/nexus/registry') {
    try {
      const { data, fresh } = await getData(env, ctx,
        ...
        nexusCore.nexusCacheKey('registry', 'r2', qs.get('query') || '', qs.get('sector') || '', qs.get('market') || ''),
  ...
  if (p === '/api/launchpad/ipos') {
    const market = (qs.get('market') || '').toUpperCase() === 'ALL' ? 'ALL' : marketCore.resolveMarketId(qs.get('market'));
    if (market === 'IN' || market === 'ALL') return json(launchpadCore.indiaResponse(launchpadSnapshot, { market, query: qs.get('q'), id: qs.get('id'), usEvents: market === 'ALL' ? launchpadSnapshot.markets.US.records : [], registry: nexusSnapshot.companies, relationships: nexusSnapshot.relationships }));
    if (!env.FINNHUB_API_KEY) return json({ schemaVersion: launchpadCore.SCHEMA_VERSION, market, unavailable: true, truth: 'UNAVAILABLE', reason: 'Finnhub IPO calendar is not configured.', events: [], graph: { nodes: [], edges: [] } });
    try {
      const from = new Date(Date.now() - 14 * 86400000).toISOString().slice(0, 10);
      const to = new Date(Date.now() + 45 * 86400000).toISOString().slice(0, 10);
      const { data, fresh } = await getData(env, ctx, `launchpad:${launchpadCore.SCHEMA_VERSION}:${market}:${from}:${to}`, async () => {
        const endpoint = `${FINNHUB_BASE}/calendar/ipo?from=${from}&to=${to}&token=${encodeURIComponent(env.FINNHUB_API_KEY)}`;
      ...
      const selected = data.find(e => e.id === qs.get('id')) || data[0] || null;
      return json({ schemaVersion: launchpadCore.SCHEMA_VERSION, market, truth: fresh ? 'SNAPSHOT' : 'CACHED', from, to, events: data, selectedId: selected && selected.id,
        graph: launchpadCore.buildImpactGraph(data, selected && selected.id, nexusSnapshot.companies, nexusSnapshot.relationships), fresh });
    } catch (err) { console.error('launchpad error:', err.message); return json(buildError(502, 'upstream_unavailable', 'IPO calendar unavailable for this refresh.'), 502); }
  }
  ...
  if (p === '/api/intel/candle') {
    ...
    try {
      const cacheKey = `candles:${AI_TASK_POLICY_SCHEMA_VERSION}:${symbol.toLowerCase()}:${range.toLowerCase()}`;
  ...
  if (p === '/api/intel/situation') {
    try {
      ...
      const { data, fresh } = await getData(env, ctx, `situation:${AI_TASK_POLICY_SCHEMA_VERSION}`, () => fetchSituation(env)); return json({ cached: !fresh, ...data });
    } catch (err) { return json({ error: true, message: friendlyError(err) }, 500); }
  }
  if (p === '/api/intel/instability') {
    try { const { data, fresh } = await getData(env, ctx, `instability:${AI_TASK_POLICY_SCHEMA_VERSION}`, () => fetchInstability(env)); return json({ cached: !fresh, ...data }); }
    catch (err) { return json({ error: true, message: friendlyError(err) }, 500); }
  }
  if (p === '/api/intel/chat' && request.method === 'POST') {
    try {
      ...
      if (!messages.every((message) => message && (message.role === 'user' || message.role === 'assistant') && typeof message.content === 'string' && message.content.length <= 2000)) {
        return json(buildError(400, 'invalid_chat_messages', 'Messages must be user/assistant text entries under 2000 characters.'), 400);
      }
      const latest = messages[messages.length - 1].content.trim();
      if (!latest) return json(buildError(400, 'empty_chat_message', 'The latest message is empty.'), 400);
      const symbol = typeof body.symbol === 'string' ? body.symbol.trim().toUpperCase().slice(0, 10) : '';
      const [trustedHeadlines, quote] = await Promise.all([
        symbol ? fetchCompanyHeadlines(env, symbol, 10).catch(() => []) : fetchMarketHeadlines().catch(() => []),
        symbol ? getQuoteCached(env, symbol).catch(() => null) : Promise.resolve(null),
      ]);
      ...
      if (!preparation.canGenerate) return json(fallback('The trusted context did not meet the chat evidence policy.'));
      ...
      let data;
      try {
        data = await runAIJson(env, system, userPrompt, validate, preparation);
      } catch (error) {
        return json(fallback(describeAiFailure(error, preparation.policy)));
      }
      return json(attachPolicy(preparation, {
        reply: data.reply,
  ...
  if (p === '/api/intel/report') {
    try { const { data, fresh } = await getData(env, ctx, `report:${AI_TASK_POLICY_SCHEMA_VERSION}`, () => fetchInvestmentReport(env)); return json({ cached: !fresh, ...data }); }
    catch (err) { return json({ error: true, message: friendlyError(err) }, 500); }
  }
  if (p === '/api/ai-status') {
    const providers = listProviders({ runtime: 'worker', includeInactive: true });
    const cd = await readCooldowns(env);
    return json({
      registrySchemaVersion: AI_PROVIDER_REGISTRY_SCHEMA_VERSION,
      providers: providers.map((provider) => providerHealthSnapshot(provider, _providerHealth, {
        configured: Boolean(
          (provider.bindingKey && env[provider.bindingKey]) ||
          (provider.envKey && env[provider.envKey])
        ),
        requestedModel: resolveProviderModel(provider, env),
        cooldownUntil: cd[provider.name] ? new Date(cd[provider.name]).toISOString() : null,
      })),
      active: providerPool(env).length,
    });
  }
  if (p === '/api/intel/alerts') {
    let recent = [];
    try { const raw = await env.MT_KV.get(ALERT_STATE_KEY); if (raw) recent = JSON.parse(raw).recent || []; } catch {}
    return json({ enabled: pushEnabled(env), policySchemaVersion: AI_TASK_POLICY_SCHEMA_VERSION, alerts: recent });
  }
  ...
  if (p === '/api/intel/priceaction') {
    ...
    const cacheKey = `cache:priceaction:${AI_TASK_POLICY_SCHEMA_VERSION}:${sym.toUpperCase()}`;
    const cached = await env.MT_KV.get(cacheKey).catch(() => null);
...
const INFRA_PIPELINES = [
  ...
  { id: 'trans-alaska', name: 'Trans-Alaska Pipeline System (TAPS)', type: 'oil', status: 'operational',
    capacity_kbd: 2100, substance: 'crude', operator: 'Alyeska Pipeline Service',
    beneficiaries: ['USA (West Coast refiners)'], risks: ['Earthquake risk; aging; Arctic wildlife corridor'],
    coords: [[-148.5,70.3],[-148.3,67.0],[-146.5,64.9],[-145.5,63.7],[-147.1,61.1]] },
  { id: 'enbridge-mainline', name: 'Enbridge Mainline', type: 'oil', status: 'operational',
    capacity_kbd: 3100, substance: 'crude', operator: 'Enbridge',
    beneficiaries: ['Canada','USA (Midwest)'], risks: ['Line 5 Michigan controversy; Indigenous land disputes'],
    coords: [[-114.1,53.5],[-104.0,50.0],[-96.8,49.0],[-89.5,46.5],[-84.0,43.5],[-82.0,42.3],[-83.0,41.7]] },
  { id: 'transco', name: 'Transcontinental Gas Pipe Line (Transco)', type: 'gas', status: 'operational',
    capacity_bcmd: 85, substance: 'natural gas', operator: 'Williams Companies',
    beneficiaries: ['USA (Gulf Coast to NYC corridor)'], risks: ['Bottleneck for Northeast winter demand'],
    coords: [[-95.4,29.7],[-88.0,30.6],[-84.4,33.7],[-79.9,32.8],[-77.0,38.9],[-74.2,40.7],[-73.8,40.8]] },
  { id: 'rex', name: 'Rockies Express Pipeline (REX)', type: 'gas', status: 'operational',
    capacity_bcmd: 27, substance: 'natural gas', operator: 'Tallgrass Energy',
    beneficiaries: ['USA (Rockies to Midwest/East)'], risks: ['Basis spread volatility; competes with Appalachian gas'],
    coords: [[-110.5,41.8],[-105.0,40.5],[-100.0,40.2],[-95.0,39.8],[-89.0,39.5],[-83.5,39.8],[-81.5,39.9]] },
  { id: 'lc-express', name: 'Louisiana-to-Canada Express (Vector)', type: 'gas', status: 'operational',
    capacity_bcmd: 15, substance: 'natural gas', operator: 'DTE/Enbridge',
    beneficiaries: ['USA Midwest','Canada'], risks: ['Aging segments; regulatory review'],
    coords: [[-87.6,41.8],[-84.5,41.5],[-83.5,42.3],[-82.5,42.9],[-79.4,43.7]] },
  { id: 'permian-highway', name: 'Permian Highway Pipeline', type: 'gas', status: 'operational',
    capacity_bcmd: 68, substance: 'natural gas', operator: 'Kinder Morgan',
    beneficiaries: ['USA (Permian Basin to Gulf/Mexico)'], risks: ['Environmental opposition; permitting delays'],
    coords: [[-102.5,31.9],[-100.0,30.4],[-97.7,30.3],[-95.4,29.7]] },
  { id: 'colonial', name: 'Colonial Pipeline', type: 'oil', status: 'operational',
    capacity_kbd: 2500, substance: 'refined products', operator: 'Colonial Pipeline Co.',
    beneficiaries: ['USA Southeast & East Coast'], risks: ['Cyberattack vector (2021 ransomware shutdown)'],
    coords: [[-84.4,33.7],[-83.0,32.1],[-80.9,33.9],[-78.6,35.8],[-77.0,38.9],[-75.1,39.9],[-74.2,40.7]] },
  { id: 'camisea', name: 'Camisea Gas Pipeline', type: 'gas', status: 'operational',
    capacity_bcmd: 12, substance: 'natural gas', operator: 'TGP/Pluspetrol',
    beneficiaries: ['Peru','LNG export'], risks: ['Amazon rainforest corridor; indigenous community conflicts'],
    coords: [[-73.5,-11.8],[-75.0,-12.5],[-76.5,-12.0],[-77.2,-11.0],[-77.1,-12.1]] },
  { id: 'gasbol', name: 'Bolivia-Brazil Gas Pipeline (GASBOL)', type: 'gas', status: 'operational',
    capacity_bcmd: 30, substance: 'natural gas', operator: 'TBG/YPFB',
    beneficiaries: ['Bolivia','Brazil (São Paulo industries)'], risks: ['Contract disputes; Bolivia nationalization history'],
    coords: [[-63.2,-17.8],[-60.0,-20.5],[-57.5,-20.5],[-55.0,-23.5],[-51.2,-23.5],[-46.6,-23.5]] },
  // ── Europe ────────────────────────────────────────────────────────────
  { id: 'nord-stream-1', name: 'Nord Stream 1', type: 'gas', status: 'sabotaged',
    capacity_bcmd: 55, substance: 'natural gas', operator: 'Gazprom',
    beneficiaries: ['Russia (formerly)','Germany (formerly)'], risks: ['Sabotaged Sep 2022; offline indefinitely'],
    coords: [[28.0,57.5],[20.0,56.5],[15.0,55.5],[13.0,54.4],[13.4,54.1]] },
  { id: 'yamal-europe', name: 'Yamal-Europe Pipeline', type: 'gas', status: 'operational',
    capacity_bcmd: 33, substance: 'natural gas', operator: 'Gazprom/EuRoPol Gaz',
    beneficiaries: ['Russia','Belarus','Poland','Germany'], risks: ['Sanctions risk; Belarus transit disputes'],
    coords: [[68.0,68.0],[56.0,62.0],[40.0,55.0],[32.0,53.5],[23.0,52.4],[20.0,52.0],[14.0,52.5],[12.5,52.5]] },
  { id: 'opal', name: 'OPAL Pipeline', type: 'gas', status: 'operational',
    capacity_bcmd: 36, substance: 'natural gas', operator: 'Wiga Transport',
    beneficiaries: ['Germany','Central Europe'], risks: ['EU capacity restrictions (antitrust)'],
    coords: [[13.4,54.1],[13.5,52.5],[13.8,51.2],[14.0,50.5],[14.5,50.0]] },
  { id: 'taggas', name: 'Trans-Austria Gas Pipeline (TAG)', type: 'gas', status: 'operational',
    capacity_bcmd: 40, substance: 'natural gas', operator: 'TAG GmbH',
    beneficiaries: ['Austria','Italy','Slovenia'], risks: ['Russian gas dependency; reverse flow constraints'],
    coords: [[16.9,48.2],[14.5,47.8],[13.0,47.8],[12.0,47.5],[11.2,46.5]] },
  { id: 'baltic-pipe', name: 'Baltic Pipe', type: 'gas', status: 'operational',
    capacity_bcmd: 10, substance: 'natural gas', operator: 'Gaz-System/Energinet',
    beneficiaries: ['Norway','Denmark','Poland'], risks: ['Limited capacity vs demand; first non-Russian route for Poland'],
    coords: [[5.0,58.0],[8.0,57.0],[10.0,56.0],[12.5,55.5],[14.5,54.0],[18.5,54.5]] },
  { id: 'interconnector-uk', name: 'Interconnector UK–Belgium', type: 'gas', status: 'operational',
    capacity_bcmd: 20, substance: 'natural gas', operator: 'Interconnector (UK) Ltd',
    beneficiaries: ['UK','Belgium','Netherlands'], risks: ['Bidirectional; Brexit complicates dispatch'],
    coords: [[1.4,51.3],[2.2,51.0],[3.0,51.2],[4.3,51.2]] },
  { id: 'greenstream', name: 'Greenstream Pipeline (Libya–Italy)', type: 'gas', status: 'operational',
    capacity_bcmd: 11, substance: 'natural gas', operator: 'ENI/NOC',
    beneficiaries: ['Libya','Italy'], risks: ['Libya civil war exposure; infrastructure sabotage risk'],
    coords: [[13.2,32.9],[13.5,34.0],[12.5,37.5],[12.5,37.8],[12.3,38.1]] },
  { id: 'medgaz', name: 'Medgaz Pipeline (Algeria–Spain)', type: 'gas', status: 'operational',
    capacity_bcmd: 10.7, substance: 'natural gas', operator: 'SONATRACH/Naturgy',
    beneficiaries: ['Algeria','Spain','EU'], risks: ['Morocco-Algeria diplomatic tensions blocked alternative route'],
    coords: [[1.8,35.8],[0.5,37.5],[-0.3,38.5],[-0.4,38.8]] },
  { id: 'eastmed', name: 'EastMed Pipeline', type: 'gas', status: 'planned',
    capacity_bcmd: 10, substance: 'natural gas', operator: 'IGI Poseidon/ENI',
    beneficiaries: ['Israel','Cyprus','Greece','Italy'], risks: ['Turkey opposition; US withdrew support 2022; geopolitical complexity'],
    coords: [[34.8,31.5],[33.0,34.5],[26.0,36.0],[22.5,37.6],[20.0,38.7],[18.5,40.6],[12.5,41.9]] },
  { id: 'brua', name: 'BRUA Corridor', type: 'gas', status: 'operational',
    capacity_bcmd: 4.4, substance: 'natural gas', operator: 'Transgaz/FGSZ',
    beneficiaries: ['Bulgaria','Romania','Hungary','Austria'], risks: ['Limited throughput; key EU diversification corridor'],
    coords: [[27.0,43.9],[25.6,44.4],[24.0,45.0],[22.0,46.5],[19.0,47.5],[16.9,48.2]] },
  // ── Russia & CIS ──────────────────────────────────────────────────────
  { id: 'power-of-siberia', name: 'Power of Siberia', type: 'gas', status: 'operational',
    capacity_bcmd: 38, substance: 'natural gas', operator: 'Gazprom',
    beneficiaries: ['Russia','China'], risks: ['Geopolitical dependency; single customer (China)'],
    coords: [[119.8,52.3],[122.0,50.0],[124.0,49.5],[126.5,49.0],[128.0,48.2],[131.0,43.5]] },
  { id: 'cpc', name: 'Caspian Pipeline Consortium (CPC)', type: 'oil', status: 'operational',
    capacity_kbd: 1400, substance: 'crude', operator: 'CPC/Chevron/Rosneft',
    beneficiaries: ['Kazakhstan','Russia (Novorossiysk)','EU'],
    risks: ['Single export route for Tengiz; sanctions exposure; storm damage'],
    coords: [[53.2,46.0],[52.0,47.5],[50.5,47.8],[47.0,45.0],[45.0,44.8],[39.0,44.7],[37.8,44.7]] },
  { id: 'central-asia-china', name: 'Central Asia–China Gas Pipeline', type: 'gas', status: 'operational',
    capacity_bcmd: 55, substance: 'natural gas', operator: 'CNPC',
    beneficiaries: ['Turkmenistan','Uzbekistan','Kazakhstan','China'],
    risks: ['Geopolitical leverage; Turkmen dependency on single buyer'],
    coords: [[61.8,37.9],[66.0,39.5],[68.0,38.5],[72.0,39.5],[75.9,39.5],[80.0,40.5],[87.6,41.8]] },
  // ── Middle East & Africa ──────────────────────────────────────────────
  { id: 'arab-gas', name: 'Arab Gas Pipeline', type: 'gas', status: 'partial',
    capacity_bcmd: 10, substance: 'natural gas', operator: 'GASCO/Jordan/Syria',
    beneficiaries: ['Egypt','Jordan','Syria','Lebanon'], risks: ['Syria conflict; segments non-functional; IS sabotage history'],
    coords: [[32.3,29.9],[35.0,29.5],[35.7,32.0],[35.5,33.9],[36.3,34.5],[37.2,36.5],[36.3,37.0]] },
  { id: 'eacop', name: 'East African Crude Oil Pipeline (EACOP)', type: 'oil', status: 'construction',
    capacity_kbd: 246, substance: 'crude', operator: 'TotalEnergies/CNOOC',
    beneficiaries: ['Uganda','Tanzania','EU investors'], risks: ['Environmental impact; community displacement; investor withdrawals'],
    coords: [[31.5,0.3],[32.0,-1.0],[33.0,-3.0],[35.5,-4.5],[37.5,-5.5],[39.0,-5.0],[39.2,-6.8]] },
  { id: 'chad-cameroon', name: 'Chad–Cameroon Pipeline', type: 'oil', status: 'operational',
    capacity_kbd: 225, substance: 'crude', operator: 'ExxonMobil/Petronas',
    beneficiaries: ['Chad','Cameroon'], risks: ['Revenue corruption; Chad political instability'],
    coords: [[15.0,13.0],[14.5,10.5],[14.0,8.0],[13.5,6.5],[11.5,4.0],[9.7,4.0]] },
  { id: 'mhp', name: 'Maghreb–Europe Gas Pipeline', type: 'gas', status: 'operational',
    capacity_bcmd: 12, substance: 'natural gas', operator: 'SONATRACH/Metragaz',
    beneficiaries: ['Algeria','Morocco','Spain','Portugal'], risks: ['Morocco-Algeria rift caused partial shutdown 2021'],
    coords: [[6.1,36.8],[3.0,36.8],[-1.8,35.4],[-5.0,35.8],[-5.6,36.0],[-8.0,38.7],[-9.1,38.7]] },
  { id: 'nigeria-loop', name: 'Nigeria Petroleum Pipeline Network', type: 'oil', status: 'operational',
    capacity_kbd: 450, substance: 'crude/refined', operator: 'NNPC',
    beneficiaries: ['Nigeria'], risks: ['Bunkering/theft (800 kbd lost); sabotage; poor maintenance'],
    coords: [[5.0,4.8],[6.5,5.5],[7.5,4.5],[8.2,4.6],[7.0,5.8],[6.8,6.5],[3.4,6.5]] },
  // ── Asia-Pacific ──────────────────────────────────────────────────────
  { id: 'sino-myanmar', name: 'China–Myanmar Pipeline', type: 'oil', status: 'operational',
    capacity_kbd: 440, substance: 'crude', operator: 'CNPC',
    beneficiaries: ['China (Yunnan bypass of Malacca)','Myanmar'],
    risks: ['Myanmar coup 2021; ethnic conflict along route'],
    coords: [[92.5,20.0],[95.5,22.0],[97.5,23.5],[100.0,25.0],[102.5,25.0],[105.0,25.5]] },
  { id: 'iran-turkey', name: 'Iran–Turkey Gas Pipeline (ITGI)', type: 'gas', status: 'operational',
    capacity_bcmd: 10, substance: 'natural gas', operator: 'NIGC/BOTAŞ',
    beneficiaries: ['Iran','Turkey'], risks: ['Sanctions on Iran; PKK sabotage history in eastern Turkey'],
    coords: [[48.3,38.1],[44.0,39.5],[40.0,39.7],[36.0,38.5],[32.5,40.9],[28.8,41.0]] },
  { id: 'tangguh-lng', name: 'Tangguh LNG Export Line', type: 'gas', status: 'operational',
    capacity_bcmd: 7.6, substance: 'LNG', operator: 'BP Indonesia',
    beneficiaries: ['Indonesia','Japan','South Korea','China'],
    risks: ['Papua political unrest; remote location logistics'],
    coords: [[134.0,-2.0],[133.5,-1.5],[130.0,0.5]] },
  { id: 'australia-darwin', name: 'Amadeus Basin Pipeline', type: 'gas', status: 'operational',
    capacity_bcmd: 1.4, substance: 'natural gas', operator: 'Jemena',
    beneficiaries: ['Australia (NT)'], risks: ['Single line supplying Darwin; no redundancy'],
    coords: [[133.9,-23.7],[133.5,-21.0],[132.5,-18.0],[131.0,-12.5],[130.9,-12.4]] },
  { id: 'gail-mainline', name: 'GAIL HVJ Mainline (India)', type: 'gas', status: 'operational',
    capacity_bcmd: 33.4, substance: 'natural gas', operator: 'GAIL India',
    beneficiaries: ['India (Hazira-Vijaipur-Jagdishpur corridor)'],
    risks: ['Aging; ROU encroachment; fertilizer sector dependency'],
    coords: [[72.6,21.1],[74.0,22.5],[76.0,23.5],[78.0,25.0],[80.0,26.8],[81.5,28.6],[83.0,27.0],[85.3,23.7]] },
];
...
const INFRA_CABLES = [
  ...
  { id: 'ellalink', name: 'EllaLink', type: 'undersea_cable', status: 'active',
    capacity_tbps: 72, owner: 'EllaLink/Telefonica',
    beneficiaries: ['Brazil','Portugal','Spain','EU'],
    risks: ['Sole direct Brazil-Europe route; Mid-Atlantic routing dependency'],
    coords: [[-46.6,-23.5],[-28.0,-20.0],[-15.0,15.0],[-10.0,38.5],[-8.7,41.5]] },
  { id: 'monet', name: 'Monet (Google)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 64, owner: 'Google/Algar/Antel',
    beneficiaries: ['Brazil','Uruguay','USA'],
    risks: ['Southern cone route; political stability'],
    coords: [[-43.2,-22.9],[-38.5,-20.0],[-30.0,-15.0],[-25.0,5.0],[-80.1,25.8],[-84.4,10.0],[-77.1,25.8]] },
  { id: 'firmina', name: 'Firmina (Google)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 350, owner: 'Google',
    beneficiaries: ['USA','Brazil','Uruguay','Argentina'],
    risks: ['South America political shifts; single owner'],
    coords: [[-74.0,40.7],[-40.0,20.0],[-35.0,0.0],[-40.0,-20.0],[-43.2,-22.9],[-52.0,-33.0],[-58.4,-34.6]] },
  { id: 'tat-14', name: 'TAT-14', type: 'undersea_cable', status: 'active',
    capacity_tbps: 3.2, owner: 'AT&T/Deutsche Telekom/BT Consortium',
    beneficiaries: ['USA','UK','Germany','Netherlands','France','Denmark'],
    risks: ['Aging; trawl risk in North Sea; 2001-era capacity'],
    coords: [[-74.0,40.7],[-50.0,45.0],[-20.0,50.0],[-8.0,51.9],[2.0,51.5],[4.0,52.0],[9.0,56.5],[10.0,57.5]] },
  { id: 'hibernia-express', name: 'Hibernia Express', type: 'undersea_cable', status: 'active',
    capacity_tbps: 8.72, owner: 'GTT Communications',
    beneficiaries: ['USA (NY)','UK (London)','Ireland'], risks: ['Low-latency finance route; targeted by HFT; single customer dependency'],
    coords: [[-74.0,40.7],[-55.0,47.0],[-30.0,52.0],[-13.8,53.0],[-8.0,51.9]] },
  { id: 'apollo', name: 'Apollo Cable System', type: 'undersea_cable', status: 'active',
    capacity_tbps: 3.2, owner: 'Cable & Wireless/Alcatel',
    beneficiaries: ['USA','UK'],
    risks: ['Aging infrastructure; 2003-era'],
    coords: [[-74.0,40.7],[-40.0,42.0],[-15.0,50.0],[-5.0,50.5],[-4.0,50.5]] },
  // ── Indian Ocean / Middle East ─────────────────────────────────────────
  { id: 'aae-1', name: 'AAE-1', type: 'undersea_cable', status: 'active',
    capacity_tbps: 40, owner: 'Consortium (China Unicom, Telia, etc.)',
    beneficiaries: ['Asia','Middle East','Africa','Europe'], risks: ['Red Sea chokepoint; Houthi threat zone'],
    coords: [[121.5,31.2],[110.4,20.0],[103.8,1.4],[80.3,13.1],[72.8,18.9],[45.3,12.5],[43.1,11.6],[36.8,11.8],[32.5,31.2],[32.3,30.1],[14.0,40.8],[12.5,41.9]] },
  { id: 'sea-me-we-3', name: 'SEA-ME-WE 3', type: 'undersea_cable', status: 'active',
    capacity_tbps: 0.96, owner: 'France Telecom/SingTel Consortium (40 members)',
    beneficiaries: ['Asia','South Asia','Middle East','Africa','Europe'],
    risks: ['Aging 1999 cable; multiple Red Sea cuts risk; 39 landing points'],
    coords: [[145.0,35.0],[135.0,34.5],[130.0,33.5],[120.0,25.0],[110.0,20.0],[100.0,5.0],[80.0,6.0],[75.0,18.0],[68.0,22.5],[55.0,25.0],[44.0,12.0],[35.0,12.5],[32.5,29.0],[28.0,32.0],[14.0,40.8],[7.0,43.5],[2.3,48.9]] },
  { id: 'sea-me-we-4', name: 'SEA-ME-WE 4', type: 'undersea_cable', status: 'active',
    capacity_tbps: 1.28, owner: 'Orange/SingTel/Consortium',
    beneficiaries: ['Singapore','India','Middle East','Europe'],
    risks: ['Multiple cut incidents; single cable linking critical nodes'],
    coords: [[103.8,1.4],[79.9,6.8],[72.8,18.9],[58.0,23.0],[44.0,12.0],[43.0,11.5],[43.0,11.3],[32.5,29.5],[29.5,35.5],[14.0,40.8],[3.0,43.3],[2.3,48.9],[-8.7,41.5]] },
  { id: 'sea-me-we-5', name: 'SEA-ME-WE 5', type: 'undersea_cable', status: 'active',
    capacity_tbps: 24, owner: 'Orange/SingTel Consortium',
    beneficiaries: ['Singapore','South Asia','Middle East','Europe'],
    risks: ['Monsoon anchor drags; Gulf of Aden piracy zone'],
    coords: [[103.8,1.4],[82.3,6.9],[73.0,18.9],[55.4,23.6],[43.3,11.5],[32.5,31.0],[14.0,40.8],[2.3,48.9],[-8.7,41.5]] },
  { id: 'imewe', name: 'IMEWE', type: 'undersea_cable', status: 'active',
    capacity_tbps: 3.84, owner: 'Consortium (STC, du, Telecom Italia)',
    beneficiaries: ['India','Middle East','West Europe'],
    risks: ['Red Sea/Suez disruption risk (Houthi attacks 2024)'],
    coords: [[72.8,18.9],[60.0,22.0],[45.0,12.0],[32.5,29.5],[32.3,31.2],[14.0,40.8],[2.3,48.9]] },
  { id: 'imewe2', name: 'Blue Raman (Google)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 288, owner: 'Google',
    beneficiaries: ['India','Saudi Arabia','Jordan','Italy'],
    risks: ['Red Sea geopolitical exposure; first non-suez Indian cable'],
    coords: [[72.8,18.9],[58.0,22.5],[39.0,22.0],[35.5,29.5],[35.0,30.5],[34.5,31.8],[34.9,32.8],[14.5,40.8],[12.5,41.9]] },
  // ── Africa ────────────────────────────────────────────────────────────
  { id: 'africa-coast-europe', name: 'Africa Coast to Europe (ACE)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 5.12, owner: 'Orange/MTN Consortium',
    beneficiaries: ['38 African countries','Europe'],
    risks: ['Multiple landing points near conflict zones'],
    coords: [[2.3,48.9],[0.0,45.0],[-8.0,38.7],[-17.0,28.5],[-17.4,14.7],[-10.8,6.3],[2.4,5.6],[9.4,3.9],[13.7,-4.3],[18.4,-33.9]] },
  { id: 'eassy', name: 'EASSy (East Africa Submarine System)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 4.72, owner: 'Consortium (Ethiopia Telecom, Telkom SA, etc.)',
    beneficiaries: ['East Africa','Southern Africa'],
    risks: ['Power outages at landing stations; political instability in Ethiopia'],
    coords: [[51.1,12.2],[45.5,2.1],[41.0,-2.5],[40.0,-8.0],[40.5,-11.4],[40.7,-15.0],[35.3,-19.8],[36.9,-22.9],[32.5,-25.9],[31.0,-29.9],[28.0,-33.0],[18.4,-33.9]] },
  { id: 'seacom', name: 'SEACOM', type: 'undersea_cable', status: 'active',
    capacity_tbps: 1.28, owner: 'SEACOM Ltd',
    beneficiaries: ['East Africa','Southern Africa','India','Europe'],
    risks: ['Cut incidents in Red Sea; East Africa last-mile monopolies'],
    coords: [[28.0,-33.0],[35.0,-22.0],[40.0,-11.0],[44.0,-2.0],[45.5,2.0],[51.1,12.2],[72.8,18.9],[80.0,13.0],[80.3,8.0],[80.2,6.8]] },
  { id: 'wacs', name: 'WACS (West Africa Cable System)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 5.12, owner: 'MTN/Vodacom/Portugal Telecom Consortium',
    beneficiaries: ['West Africa','South Africa','Europe'],
    risks: ['Cable cuts off West Africa coast; trawling damage'],
    coords: [[2.3,48.9],[-8.7,41.5],[-17.0,28.5],[-17.4,14.7],[-17.5,11.9],[-13.7,9.5],[-10.8,6.3],[-1.6,5.6],[3.5,4.0],[8.5,4.1],[9.0,1.0],[9.7,-4.0],[12.3,-4.3],[13.2,-8.8],[13.2,-14.9],[14.5,-22.9],[18.4,-33.9]] },
  { id: 'mainone', name: 'MainOne', type: 'undersea_cable', status: 'active',
    capacity_tbps: 1.92, owner: 'MainOne Cable Company',
    beneficiaries: ['Nigeria','Ghana','Portugal'],
    risks: ['Cut 2020 left Nigeria without 25% of capacity'],
    coords: [[-9.1,38.7],[-17.0,28.5],[-17.5,11.0],[-10.8,6.3],[-1.6,5.6],[3.5,5.5]] },
  // ── Trans-Pacific ─────────────────────────────────────────────────────
  { id: 'dunant', name: 'Dunant (Google)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 250, owner: 'Google',
    beneficiaries: ['USA','France'], risks: ['Sole owner; single transatlantic route'],
    coords: [[-74.0,40.7],[-45.0,36.0],[-10.0,44.0],[-1.6,48.7]] },
  { id: 'jupiter', name: 'Jupiter', type: 'undersea_cable', status: 'active',
    capacity_tbps: 60, owner: 'Facebook/PLDT/SoftBank',
    beneficiaries: ['USA Pacific','Japan','Philippines'],
    risks: ['Pacific seismic zones; Luzon Strait fault'],
    coords: [[-122.4,37.8],[-145.0,35.0],[145.0,35.0],[139.7,35.7],[123.9,10.3],[124.0,11.6]] },
  { id: 'echo', name: 'Echo (Google/Meta)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 480, owner: 'Google/Meta',
    beneficiaries: ['USA','Singapore','Guam'],
  ...
  { id: 'havfrue', name: 'HAVFRUE / AEC-1', type: 'undersea_cable', status: 'active',
    ...
    risks: ['Arctic routing; trawl damage risk'],
    coords: [[-74.0,40.7],[-45.0,52.0],[-13.8,53.0],[-8.0,51.9],[10.0,57.9],[10.7,59.9]] },
  { id: 'faster', name: 'FASTER', type: 'undersea_cable', status: 'active',
    capacity_tbps: 60, owner: 'Google/KDDI/China Mobile Consortium',
    beneficiaries: ['USA','Japan'],
    risks: ['Japan-US; seismic risk along Ring of Fire'],
    coords: [[-122.4,37.8],[-155.0,30.0],[-175.0,28.0],[155.0,32.0],[139.7,35.7],[136.5,34.7]] },
  { id: 'sjc2', name: 'SJC2 (Southeast Asia–Japan Cable 2)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 126, owner: 'SoftBank/Google/Facebook/Amazon Consortium',
    beneficiaries: ['Japan','South Korea','China','Philippines','Singapore'],
    risks: ['South China Sea territorial disputes; fishing trawl damage'],
    coords: [[139.7,35.7],[129.0,35.0],[124.0,24.0],[121.0,16.0],[120.0,10.0],[119.0,6.0],[103.8,1.4]] },
  { id: 'apg', name: 'APG (Asia Pacific Gateway)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 54.8, owner: 'China Telecom/CUC/KT/SoftBank Consortium',
    beneficiaries: ['China','South Korea','Japan','Malaysia','Singapore'],
    risks: ['South China Sea cut risk; Chinese government influence on operator'],
    coords: [[121.5,31.2],[126.9,37.5],[135.0,34.5],[130.0,33.5],[120.5,24.0],[117.0,21.0],[114.2,22.3],[104.0,1.5],[103.8,1.4]] },
  { id: 'pacific-light-cable', name: 'Pacific Light Cable Network (PLCN)', type: 'undersea_cable', status: 'partial',
    capacity_tbps: 120, owner: 'Google/Meta (partial; China segment blocked by FCC)',
    beneficiaries: ['USA','Taiwan','Philippines'],
    risks: ['Geopolitical: US blocked Hong Kong segment (FCC 2021)'],
    coords: [[-118.2,33.8],[-155.0,25.0],[121.9,25.0],[124.0,11.6],[122.5,31.2]] },
  { id: 'bifrost', name: 'Bifrost (Meta)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 200, owner: 'Meta/Telin',
    beneficiaries: ['USA','Indonesia','Philippines','Singapore'],
    risks: ['Pacific seismic; Meta dependency concentration'],
    coords: [[-122.4,37.8],[-155.0,20.0],[145.0,13.5],[130.0,0.5],[120.0,-5.0],[115.0,-8.5],[106.8,-6.2],[103.8,1.4]] },
  { id: 'hawai-cable', name: 'Hawaiki', type: 'undersea_cable', status: 'active',
    capacity_tbps: 43.8, owner: 'Hawaiki Submarine Cable',
    beneficiaries: ['USA','New Zealand','Australia'],
    risks: ['Pacific seismic; single route for NZ international bandwidth'],
    coords: [[-122.4,37.8],[-155.0,20.0],[-170.0,10.0],[178.0,-17.0],[174.8,-37.0],[153.0,-27.5],[151.2,-33.9]] },
  // ── Intra-regional ────────────────────────────────────────────────────
  { id: 'apcn-2', name: 'APCN-2', type: 'undersea_cable', status: 'active',
    capacity_tbps: 2.56, owner: 'AT&T/KDD Consortium',
    beneficiaries: ['Asia-Pacific ring'], risks: ['Aging; South China Sea territorial disputes'],
    coords: [[139.7,35.7],[129.0,35.1],[122.5,31.2],[117.2,39.9],[126.9,37.5],[139.7,35.7]] },
  { id: 'south-atlantic-express', name: 'South Atlantic Express (SAEx)', type: 'undersea_cable', status: 'planned',
    capacity_tbps: 60, owner: 'SAEx International',
    beneficiaries: ['Brazil','South Africa'],
    risks: ['Financing uncertainty; first direct South Atlantic route'],
    coords: [[-43.2,-22.9],[-25.0,-30.0],[-5.0,-35.0],[18.4,-33.9]] },
  { id: 'basslink', name: 'Basslink (Australia–Tasmania)', type: 'undersea_cable', status: 'active',
    capacity_tbps: 0.63, owner: 'Basslink Pty Ltd',
    beneficiaries: ['Australia mainland','Tasmania'], risks: ['2016 failure left Tasmania on diesel; single interconnect'],
    coords: [[146.8,-39.2],[147.5,-41.0],[147.3,-42.9]] },
];
...
const INFRA_ROUTES = [
  ...
  // ── Additional straits & chokepoints ─────────────────────────────────
  { id: 'gibraltar', name: 'Strait of Gibraltar', type: 'trade_route',
    throughput_ships_day: 300, cargo_types: ['LNG','Oil','Containers','Bulk'],
    beneficiaries: ['Mediterranean nations','Global trade'],
    risks: ['NATO southern flank; Morocco-Spain territorial tensions (Ceuta)'],
    coords: [[-6.0,36.0],[-5.4,35.9],[-5.0,36.0],[-4.5,36.2],[-3.5,36.5]] },
  { id: 'lombok', name: 'Lombok Strait (Malacca alternative)', type: 'trade_route',
    throughput_ships_day: 30, cargo_types: ['VLCC tankers','LNG (deep water route)'],
    beneficiaries: ['China','Japan','Oil tanker operators avoiding Malacca draft limits'],
    risks: ['Longer route; Indonesian EEZ jurisdiction'],
    coords: [[116.4,-8.8],[116.0,-8.5],[115.8,-8.2],[115.7,-7.9]] },
  { id: 'sunda', name: 'Sunda Strait', type: 'trade_route',
    throughput_ships_day: 15, cargo_types: ['Oil','Bulk','Containers'],
    beneficiaries: ['Indonesia','Australia','Indian Ocean-Pacific shipping'],
    risks: ['Krakatau volcanic activity; tsunami risk; shallow in places'],
    coords: [[106.0,-6.0],[105.9,-6.3],[105.8,-6.8],[105.7,-7.2]] },
  { id: 'mozambique-channel', name: 'Mozambique Channel', type: 'trade_route',
    throughput_ships_day: 25, cargo_types: ['Oil (VLCC)','LNG','Bulk'],
    beneficiaries: ['South African ports','Indian Ocean routing'],
    risks: ['Piracy risk; cyclone season; Mozambique insurgency north'],
    coords: [[40.0,-12.0],[38.0,-16.0],[36.0,-20.0],[34.0,-24.0],[32.0,-28.0]] },
  { id: 'northwest-passage', name: 'Arctic Northwest Passage', type: 'trade_route',
    throughput_ships_day: 1, cargo_types: ['LNG','Bulk','Seasonal'],
    beneficiaries: ['Canada','USA','Arctic resource shipping'],
    risks: ['Seasonal ice; climate change opens route; Canadian sovereignty dispute with USA'],
    coords: [[-80.0,75.0],[-90.0,74.0],[-100.0,72.5],[-110.0,70.5],[-120.0,69.5],[-135.0,69.0],[-155.0,70.5],[-167.0,66.0]] },
  { id: 'tsushima', name: 'Korea/Tsushima Strait', type: 'trade_route',
    throughput_ships_day: 120, cargo_types: ['Containers','Cars','LNG'],
    beneficiaries: ['Japan','South Korea','Global supply chain'],
    risks: ['North Korea missile tests; Japan Sea military incidents'],
    coords: [[129.4,34.5],[129.6,34.9],[129.8,35.3],[130.0,35.8],[130.5,36.5]] },
  { id: 'luzon', name: 'Luzon Strait', type: 'trade_route',
    throughput_ships_day: 45, cargo_types: ['Containers','Oil','LNG'],
    beneficiaries: ['China','Japan','Philippines'],
    risks: ['South China Sea militarisation; typhoon belt; submarine cable concentration'],
    coords: [[122.0,21.5],[121.5,20.5],[121.0,19.5],[120.5,18.5]] },
  // ── Major ocean corridors ─────────────────────────────────────────────
  { id: 'transpacific-north', name: 'North Pacific Trade Corridor', type: 'trade_route',
    throughput_ships_day: 50, cargo_types: ['Containers','Electronics','Vehicles'],
    beneficiaries: ['USA West Coast','China','Japan','South Korea'],
    risks: ['Trans-Pacific shipping lane; container rate volatility; 2021 port congestion'],
    coords: [[121.5,31.2],[135.0,35.0],[155.0,38.0],[175.0,40.0],[-170.0,42.0],[-155.0,40.0],[-130.0,38.0],[-118.2,33.8]] },
  { id: 'transpacific-south', name: 'South Pacific Trade Corridor', type: 'trade_route',
    throughput_ships_day: 25, cargo_types: ['Bulk minerals','LNG','Containers'],
    beneficiaries: ['Australia','New Zealand','South America','Asia'],
    risks: ['Long routing; cyclone season; limited port infrastructure'],
    coords: [[151.2,-33.9],[170.0,-35.0],[-170.0,-30.0],[-130.0,-25.0],[-80.0,-15.0],[-77.1,-12.1]] },
  { id: 'transatlantic-north', name: 'North Atlantic Trade Route', type: 'trade_route',
    throughput_ships_day: 120, cargo_types: ['Containers','Oil','LNG'],
    beneficiaries: ['USA East Coast','UK','Northern Europe'],
    risks: ['Iceberg risk; North Atlantic storms; aging port infrastructure (Rotterdam)'],
    coords: [[-74.0,40.7],[-50.0,44.0],[-30.0,50.0],[-15.0,51.0],[-8.0,51.9],[2.0,51.5],[4.5,52.3]] },
  { id: 'indian-ocean-east', name: 'Indian Ocean Eastern Route', type: 'trade_route',
    throughput_ships_day: 60, cargo_types: ['Oil (Gulf to Asia)','LNG','Iron ore'],
    beneficiaries: ['India','China','Japan','South Korea'],
    risks: ['Gulf of Aden piracy; Andaman Sea political tension'],
    coords: [[56.3,23.4],[65.0,20.0],[72.0,15.0],[78.0,10.0],[82.0,6.0],[89.0,5.0],[95.0,4.0],[100.0,3.0],[103.8,1.4]] },
  { id: 'cape-horn', name: 'Cape Horn Route', type: 'trade_route',
    throughput_ships_day: 10, cargo_types: ['Bulk','LNG','VLCC'],
    beneficiaries: ['Atlantic-Pacific routing without Panama Canal'],
    risks: ['Notorious storms; iceberg risk; extreme weather; only viable for large VLCC'],
    coords: [[-65.0,-50.0],[-66.0,-54.0],[-67.5,-55.5],[-68.0,-55.9],[-67.0,-57.0],[-65.0,-56.0]] },
  { id: 'gulf-mexico', name: 'Gulf of Mexico Oil Routes', type: 'trade_route',
    throughput_ships_day: 140, cargo_types: ['Crude oil','LNG','Refined products'],
    beneficiaries: ['USA refiners','Mexico','Caribbean'],
    risks: ['Hurricane season; offshore platform vulnerability'],
    coords: [[-97.0,26.0],[-94.0,27.0],[-90.0,28.5],[-88.0,28.5],[-85.0,29.0],[-81.0,30.0],[-80.0,25.0]] },
  { id: 'caribbean-windward', name: 'Caribbean Windward Passage', type: 'trade_route',
    throughput_ships_day: 55, cargo_types: ['Containers (East Coast US via Panama)','Oil'],
    beneficiaries: ['USA East Coast','Caribbean nations'],
    risks: ['Cuba geopolitical proximity; drug trafficking corridor'],
    coords: [[-74.5,20.0],[-73.8,19.5],[-73.0,18.8],[-72.5,18.5]] },
  { id: 'persian-gulf-route', name: 'Persian Gulf Internal Routes', type: 'trade_route',
    throughput_ships_day: 80, cargo_types: ['Crude oil','LNG','Refined products'],
    beneficiaries: ['Gulf states','Global oil markets'],
    risks: ['Iran-GCC tensions; mine threat; Hormuz proximity'],
...
async function fetchPriceAction(env, symbol) {
  const [quoteResult, newsResult] = await Promise.allSettled([
    ...
    fetchCompanyHeadlines(env, symbol, 8),
  ]);
...
Return ONLY the JSON object. No markdown, no commentary.`;
  ...
  try {
    const explanation = await runAIJson(
      env,
