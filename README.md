...
- Checkpoint 7 source commit `febefb3` fixes company evidence attribution/ingestion, reserves verifier budget before generation, aligns lower-risk output limits, and exposes safe failure-attempt diagnostics. It served all seven `20260730a` assets and passed `33/33` production contracts. Targeted probes confirmed richer company evidence and fail-closed provider/verifier behavior, but also exposed an undersized one-card NEWS result; the `2026-07-30b` follow-up rejects undersized enrichment, moves NEWS to a fresh cache namespace, and returns the canonical source-linked fallback instead. That follow-up is locally verified with `50/50` unit tests, `31/31` available smoke contracts, all offline AI thresholds, zero dependency vulnerabilities, and Worker dry-run bundling; final production verification is pending.
