...
- Checkpoint 8 source commits `9370cc3` and `953c0ea` restore Deep Dive as an always-useful, data-first deterministic dossier. Schema `2026-08-04a` uses the pooled quote cascade, source-balanced company evidence, fundamentals, analyst counts, provenance, and watch items; `20260804b` separates optional `ai=1` verification so it cannot block ready data. Both source checkpoints are production verified: the final source served all seven assets, passed `33/33`, returned fresh AAPL/Apple/MSFT dossiers in 1.06-1.35 seconds, and passed desktop/mobile browser checks without console errors. Optional AI remained safely withheld after failed independent verification; no positive high-risk acceptance is claimed.
- Checkpoint 9 source commit `ebacf62` repairs the screenshot-observed all-zero ticker tape and misleading Deep Dive data cards. The tape uses the pooled quote cascade plus a 24-hour last-good fallback instead of direct Finnhub-only calls. Deep Dive schema `2026-08-04b` reports equity dataset coverage and a bounded, source-linked Nasdaq options snapshot for optionable US stocks without turning coverage into an investment score or inferring a trade. Production served all seven `20260804d` assets, passed `34/34`, returned all seven positive tape prices, and rendered SPCX with 100/100 coverage plus 47 options rows on desktop/mobile without console errors.
...
- **Scrolling ticker tape** of mega-caps from the pooled quote cascade, color-coded, refreshed every 60s, with visibly marked last-good values during transient provider failures.
...
- **Resilient Deep Dive** — returns pooled quote/fundamental/analyst/news coverage plus a bounded Nasdaq chain snapshot when listed options exist; AI/provider failure withholds only the optional narrative, while unavailable IV/Greeks/trade models remain explicit.
