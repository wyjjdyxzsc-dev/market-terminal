...
- Checkpoint 9 repairs the screenshot-observed all-zero ticker tape and misleading Deep Dive data cards. The tape now uses the pooled quote cascade plus a 24-hour last-good fallback instead of direct Finnhub-only calls. Deep Dive schema `2026-08-04b` reports equity dataset coverage and a bounded, source-linked Nasdaq options-chain snapshot for optionable US stocks; it does not turn coverage into an investment score or infer a trade. Local evidence is `58/58` unit tests, `32/32` available contracts, all offline AI thresholds, zero dependency vulnerabilities, Wrangler `4.118.0` dry-run, live seven-symbol/SPCX probes, and desktop/mobile browser checks. Production is pending.
