  ...
  async function openEntity(id, marker) {
    ...
    drawer.innerHTML =
      ...
      (securities.length ? `<div class="atlas-d-sec"><div class="atlas-d-label">SECURITY</div>${securities.map((s) => {
        ...
        return `<div class="atlas-sec-row"><span class="num">${esc(exch)}:${esc(sym)}</span><span class="atlas-sec-mkt">${esc(mkt)}</span><button type="button" class="btn-sm" data-open="${esc(s)}" data-view="terminal">Terminal</button><button type="button" class="btn-sm" data-open="${esc(s)}" data-view="deepdive">Deep Dive</button></div>`;
      ...
      (d.nearbyEvents && d.nearbyEvents.length ? `<ul class="atlas-events">${d.nearbyEvents.map((ev) => `<li><span class="atlas-ev-type" style="--sw:${EVENT_COLOR[ev.type] || EVENT_COLOR.OTHER}">${esc(ev.type)}</span> ${esc(ev.title)} <small>${ev.distanceKm} km</small></li>`).join('')}</ul>` : '<p class="atlas-ev-note">No active sourced event within 250 km.</p>') +
