/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function formatAiRuntimeSummary(policy) {
  ...
  if (generator && generator.provider) {
    ...
    const providers = [...new Set(attempts.map((attempt) => attempt.provider).filter(Boolean))];
    parts.push(`no model output accepted after ${attempts.length} attempt${attempts.length === 1 ? '' : 's'}${providers.length ? ` (${providers.join(', ')})` : ''}`);
  } else {
    parts.push('no model output accepted');
  }
  if (verifier && verifier.status === 'passed') {
    parts.push(`verified by ${verifier.provider}/${verifier.servedModel || verifier.requestedModel || 'model'}`);
  } else if (verifier && verifier.status === 'rejected') {
    parts.push(`rejected by ${verifier.provider}/${verifier.servedModel || verifier.requestedModel || 'model'}`);
  } else if (verifier && verifier.status === 'deterministic') {
    parts.push('deterministic verifier');
  } else if (policy.requiresIndependentVerifier) {
    parts.push('independent verification not completed');
  }
  if (Number(totals.providerCalls) > 0) {
    parts.push(`${Number(totals.providerCalls)} call${Number(totals.providerCalls) === 1 ? '' : 's'}`);
    const estimated = runtime.generator?.usage?.estimated || runtime.verifier?.usage?.estimated;
    parts.push(`${Number(totals.totalTokens || 0).toLocaleString()} ${estimated ? 'estimated ' : ''}tokens`);
    parts.push(`${Number(totals.latencyMs || 0).toLocaleString()} ms`);
    parts.push(`${Number(totals.costUnits || 0)} cost unit${Number(totals.costUnits) === 1 ? '' : 's'}`);
  }
  if (runtime.failureCode) {
    parts.push(`failure: ${String(runtime.failureCode).replace(/_/g, ' ')}`);
  }
  return parts.join(' · ');
}
...
(function AIChatEngine() {
  ...
  function appendPolicyMeta(bubble, data) {
    ...
    if (runtime) {
      ...
      runtimeLine.className = 'chat-policy-runtime';
      runtimeLine.textContent = runtime;
      note.appendChild(runtimeLine);
    }
    ...
    try {
      ...
      if (data && data.reply) {
        ...
        try {
          ...
          console.warn('[chat] policy metadata render failed', metaError);
        }
