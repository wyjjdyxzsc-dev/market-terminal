(() => {
  ...
  function detail(e) {
    ...
    const links=(e.evidence||[]).map(x=>{ const url=safeUrl(x.url); return url?`<li><a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${esc(x.source)}</a> · ${esc(date(x.publishedAt))} · ${esc(x.credibilityClass)}</li>`:''; }).join('');
