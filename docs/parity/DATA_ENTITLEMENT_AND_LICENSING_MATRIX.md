...
- Core source commit `febefb3` passed production contracts and targeted probes without adding an entitlement. The first live NEWS probe exposed an undersized batch, and the `2026-07-30b` local follow-up changes only validation, cache isolation, and fallback behavior; its production verification is pending. No new market-data, issuer-research, options, relationship, intelligence, or professional-terminal entitlement is claimed.
