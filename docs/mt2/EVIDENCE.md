...
- Local Cron testing used Wrangler 4 `dev --local --test-scheduled` and two `/cdn-cgi/local/scheduled?format=json` invocations, both outcome `ok`; source outage did not abort the handler or erase events. Existing `0 * * * *` schedule = 24 runs/day. WORLDWIRE adds at most four source requests, one KV read and one KV write/run: ~96 source subrequests, 24 reads, 24 writes/day, plus existing hourly news KV write (total scheduled 48 writes/day). At hot bound 600 and warm bound 400, local serialized state was ~1.58 MB; write growth is one replacement value/run, not one key/article. Account plan could not be read because Wrangler auth had expired; Cloudflare published Free ceilings are used as a conservative design envelope: 100,000 Worker requests/day, 50 subrequests/invocation, 100,000 KV reads/day, 1,000 KV writes/day and 1 GB storage. No paid feature introduced. Sources are polled hourly and rate-limited failures back off. Published docs: https://developers.cloudflare.com/workers/platform/limits/ and https://developers.cloudflare.com/kv/platform/limits/.
...
- Review fixes: event ID no longer derives from title fingerprint; all-source failure preserves last successful ingest timestamp; feed response body capped at 4 MB; NWS test/demo alerts excluded; USGS only magnitude>=3 enters stream; new ranking interleaves provider groups to avoid NWS flooding. Remaining live limitation B-038/B-039.
