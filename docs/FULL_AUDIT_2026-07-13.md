...
- Checkpoint date: 2026-07-30. Core source commit `febefb3` is production verified; the `2026-07-30b` NEWS batch follow-up is locally verified and awaiting managed deployment.
...
- Follow-up local evidence: `50/50` unit tests, all 10 offline AI evaluation thresholds, `31/31` available smoke contracts, zero known dependency vulnerabilities, syntax checks, Wrangler `4.115.0` dry-run bundling at 408.24 KiB raw / 98.96 KiB gzip, and an undersized NEWS model response correctly replaced by 14 source-linked degraded items.
...
| AUD-017 | high | high | AI availability | company evidence ingestion, provider order/budgets, lower-risk output limits | AI features can abstain before valid evidence reaches the model, exhaust verifier budget, or degrade because provider output is truncated/rate-limited | Source commit `febefb3` passed `33/33` production contracts and targeted company/provider probes, but the first deployed NEWS probe exposed a one-card batch accepted from 60 inputs; the `2026-07-30b` minimum-batch/cache follow-up passed local gates | Deploy the NEWS follow-up through the managed path and verify useful-batch or truthful-fallback behavior in production | Unit, local/production contracts, targeted runtime probes, browser disclosure | partially-resolved-production-verified | febefb3 |
