...
- Checkpoint date: 2026-07-30. Local verification is complete; production verification is pending.
...
- Lower-risk Express and Worker races share a 4,000-token output ceiling. Local NEWS strict JSON failed at 2,000 tokens and the account's tokens-per-minute request ceiling rejected 8,000; the bounded midpoint returned 12 enriched, source-linked items. Per-minute throttles now use a short cooldown rather than the daily/quota cooldown.
- Local evidence: `50/50` unit tests, all 10 offline AI evaluation thresholds, `31/31` available smoke contracts, zero known dependency vulnerabilities, syntax checks, Wrangler `4.115.0` dry-run bundling at 407.58 KiB raw / 98.76 KiB gzip, targeted evidence/provider probes, and an interactive browser check passed.
...
| AUD-017 | high | high | AI availability | company evidence ingestion, provider order/budgets, lower-risk output limits | AI features can abstain before valid evidence reaches the model, exhaust verifier budget, or degrade because provider output is truncated/rate-limited | Live/local probes reproduced publisher-domain loss, missing Finnhub evidence, broad generator budget exhaustion, and NEWS JSON/TPM failures; checkpoint 7 repairs passed local unit/smoke/evaluation/browser/build checks | Deploy through managed path and verify company evidence, provider attempts, NEWS fallback/enrichment, and current-market behavior in production | Unit, local/production contracts, targeted runtime probes, browser disclosure | resolved-local-verified | |
