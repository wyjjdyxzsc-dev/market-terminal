...
- Checkpoint date: 2026-08-04. Deep Dive source commit `9370cc3` is production verified; the data-first latency follow-up is locally verified and awaiting managed production verification.
...
- Those production probes exposed a second defect: uncached Apple/MSFT responses blocked 32-37 seconds on optional AI after the deterministic dossier was ready. The `20260804b` follow-up splits the cache/API paths, returns the dossier first, runs `ai=1` verification in the UI background, and ignores stale query completions. Local `54/54` unit and `31/31` available contract checks, offline AI thresholds, zero-vulnerability audit, Wrangler `4.118.0` dry-run at 424.74 KiB raw / 102.62 KiB gzip, and desktop/mobile browser checks pass; follow-up production verification is pending.
...
- No capability row, entitlement, or positive high-risk model-quality claim is asserted. The source dossier is production verified; only the data-first latency follow-up remains production-pending.
...
| AUD-018 | high | high | Deep Dive availability | `shared/deep-dive-core.js`, company evidence merge, Deep Dive quote path/UI | A successful HTTP response appears broken when evidence loses diversity, direct quote access fails, AI abstains into empty sections, or optional verification blocks the ready dossier | Schema `2026-08-04a` builds the source-backed dossier and commit `9370cc3` passed assets, `33/33` production contracts, and targeted probes; `20260804b` separates data and optional AI requests after production exposed 32-37 second waits | Deploy the data-first follow-up and repeat strict production contract, targeted latency/state probes, assets, and browser verification | Unit, strict local/production contract, targeted probes, desktop/mobile browser | source-resolved-production-verified-followup-pending | 9370cc3 |
