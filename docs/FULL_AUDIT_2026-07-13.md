...
- Local checkpoint date: 2026-07-16. Production deployment and verification are pending.
...
| AUD-007 | high | high | AI accuracy | `server.js`, `worker.js` AI race helpers and task handlers | High-risk analyses can present unsupported claims | Shared task policies now gate seven high-risk/current-market paths, constrain unsupported financial output, and return structured abstentions; independent claim-level verification and remaining AI tasks are not yet covered | Extend policy registry to all tasks and add independent verifier plus measured AI evaluation fixtures | AI evaluation fixtures and production contract verification | partially-resolved-local-verified | |
