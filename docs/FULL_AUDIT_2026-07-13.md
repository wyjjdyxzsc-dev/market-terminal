...
- Checkpoint date: 2026-07-28. Local implementation is verified; managed production deployment and production evidence are pending.
...
| AUD-007 | high | high | AI accuracy | `server.js`, `worker.js`, shared AI policy/registry/verification/evaluation cores | High-risk analyses can present unsupported claims | Policy-bound generated tasks now require evidence/citations, a different-provider/different-model verifier, bounded calls/tokens/cost/latency, and runtime disclosure; deterministic tasks retain deterministic authority. Offline safety fixtures pass locally, but live-provider quality and production behavior are not yet verified for this checkpoint | Complete managed production verification, then add live-provider drift canaries, a human-labelled finance/OSINT claim corpus, calibration metrics, and persistent aggregate observability | Unit/evaluation fixtures, local/production contracts, targeted runtime probes, browser disclosure checks | partially-resolved-local-verified | 20c9252 |
