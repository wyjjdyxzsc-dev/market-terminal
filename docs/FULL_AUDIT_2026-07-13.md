...
| AUD-007 | high | high | AI accuracy | `server.js`, `worker.js` AI race helpers and task handlers | High-risk analyses can present unsupported claims | Eleven analytical/current-market paths are production verified under cited-heavy or deterministic authority. Independent claim-level verification and remaining prompts are not yet covered | Add independent verifier, remaining prompt inventory, provider/model telemetry, cost controls, and measured evaluation fixtures | AI evaluation fixtures and production contract verification | partially-resolved-production-verified | 20c9252 |
